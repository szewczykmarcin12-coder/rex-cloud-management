...
const C = {
  ink: '#1c2333', dark: '#39415a', mid: '#5571e0', mute: '#71798c', line: '#e3e6ed', pale: '#f7f8fa',
  ok: '#16705b', warn: '#B26A00', bad: '#a03f37', blue: '#3A6EA5', violet: '#7A5FB0', teal: '#26A69A', slate: '#455A64',
};
const CATEGORIES = [
  ['crew', 'Crew', C.blue],
  ['manager', 'MGR', C.ink],
...
function Notice({ type = 'warn', children }) {
  const style = type === 'bad' ? { bg: '#fff0ed', fg: C.bad, Icon: AlertTriangle } : type === 'ok' ? { bg: '#e8f2ef', fg: C.ok, Icon: CheckCircle2 } : { bg: '#fff8e6', fg: C.warn, Icon: AlertTriangle };
...
export default function MonthlyForecast({ api, data }) {
  ...
  return <div className="flex-1 min-h-0 overflow-y-auto" style={{ backgroundColor: C.pale }}>
    <div className="sticky top-0 z-20 flex flex-wrap items-center gap-3 border-b px-6 py-3 bg-white/95 backdrop-blur" style={{ borderColor: C.line }}>
      <div className="mr-auto"><div className="flex items-center gap-2"><TrendingUp size={20} style={{ color: C.dark }} /><h1 className="font-bold" style={{ color: C.ink }}>Forecast miesiąca i Cost of Labour</h1>{plan && <span className="rounded-full px-2 py-0.5 text-[10px] font-bold" style={{ backgroundColor: plan.status === 'LOCKED' ? '#e8f2ef' : '#fff8e6', color: plan.status === 'LOCKED' ? C.ok : C.warn }}>{plan.status === 'LOCKED' ? 'ZABLOKOWANY' : 'ROBOCZY'} · v{plan.version}</span>}</div><p className="text-[11px] mt-0.5" style={{ color: C.mute }}>Sprzedaż i transakcje → rozkład historyczny → godziny → COL → limit grafiku</p></div>
    ...
    {edit && <div className="fixed inset-0 z-[90] flex items-center justify-center p-4"><div className="absolute inset-0 bg-black/50" onClick={() => setEdit(null)} /><div className="relative bg-white rounded-2xl w-full max-w-4xl p-6 shadow-xl">
      ...
      <div className="flex justify-between gap-2 mt-5"><div>{plan.overrides?.[edit.date] && plan.status !== 'LOCKED' && <button onClick={clearDay} className="rounded-lg px-3 py-2 text-sm" style={{ backgroundColor: '#fff0ed', color: C.bad }}>Usuń korektę</button>}</div><div className="flex gap-2"><button onClick={() => setEdit(null)} className="rounded-lg px-4 py-2 text-sm" style={{ backgroundColor: C.pale, color: C.dark }}>Zamknij</button>{plan.status !== 'LOCKED' && <button onClick={saveDay} disabled={busy} className="rounded-lg px-4 py-2 text-sm font-bold text-white" style={{ backgroundColor: C.dark }}>Zapisz i przelicz</button>}</div></div>
    </div></div>}
  </div>;
}
